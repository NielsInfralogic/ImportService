...
TCHAR	g_szServiceName[MAX_PATH] = _T("ImportService");
TCHAR	g_szServiceDescription[MAX_PATH] = _T("PDFhub Plan Import Service");
BOOL	g_bRunning = TRUE;

PLUGININIT pPluginInit = NULL;
PLUGINEXIT pPluginExit = NULL;
PLUGINSETUP pPluginSetup = NULL;
PLUGINSETUP pPluginSetup2 = NULL;
PLUGINJOB pPluginJob = NULL;
PLUGINJOBEX pPluginJobEx = NULL;
PLUGINJOBSTATUS pPluginJobStatus = NULL;



extern CPrefs g_prefs;
extern CNPPrefs g_npprefs;
extern CUtils g_util;
extern HINSTANCE hPluginDll;
...
BOOL InstallService()
{
	...
	keyClasses.Create(HKEY_LOCAL_MACHINE, _T("Software\\InfraLogic\\ImportService"));
	...
	keyClasses.SetKeyValue(_T("Parameters"), _T("c:\\MessageServerService"), _T("LogFileFolder"));
	...
	keyClasses.SetKeyValue(_T("Parameters"), _T("c:\\ImportService\\Config.ini"), _T("ConfigFile"));
...
void WINAPI ServiceMain(DWORD argc, LPTSTR *argv)
{
	...
	strcpy(szEventStr, "ImportService");
	LogEvent(EVENTLOG_SUCCESS, IDS_STARTEDOK, szEventStr,NULL,NULL);


		////////////////////////////////////////////////
		// Here is the actual message waiting loop
		////////////////////////////////////////////////

	if (g_prefs.LoadPreferencesFromRegistry() == FALSE)
		g_util.Logprintf("CANNOT READ Registry: Software\\InfraLogic\\ImportService\\Parameters..\r\n");
	else {
		g_util.Logprintf("INFO: Registry read: Software\\InfraLogic\\ImportService\\Parameters..\r\n");
	...
	g_util.Logprintf("Service started - reading database setups..");
	CString sIniFile = g_util.GetTempFolder() + _T("\\ImportService") + g_util.Int2String(g_prefs.m_instancenumber) + _T(".ini");
	CString sIniFile2 = g_util.GetTempFolder() + _T("\\NPImportService") + g_util.Int2String(g_prefs.m_instancenumber) + _T(".ini");
	
	if (dbmaint.LoadImportConfigurations(1 /*g_prefs.m_instancenumber*/, sErrorMessage) == FALSE) {
		g_util.Logprintf("ERROR: LoadImportConfigurations() - " + sErrorMessage);
	}

	if (g_prefs.m_ImportConfigurations.GetCount() == 0)
		g_util.Logprintf("ERROR: No ImportConfigurations found. ");		
	else
	{
		

		LogEvent(EVENTLOG_SUCCESS, IDS_CONNDB2, szEventStr, NULL, NULL);
		g_prefs.LoadIniFile(g_prefs.m_ImportConfigurations[0].sConfigFile);
		g_util.Logprintf("INFO: Config file %s (%d) read:\r\n", g_prefs.m_ImportConfigurations[0].sConfigFile, g_prefs.m_instancenumber);
		g_npprefs.LoadIniFile(g_prefs.m_ImportConfigurations[0].sConfigFile2);
		g_util.Logprintf("INFO: Config file %s read:\r\n", g_prefs.m_ImportConfigurations[0].sConfigFile2);

		// path folders..
		g_prefs.m_inputfolder = g_prefs.m_ImportConfigurations[0].sInputFolder;
		g_prefs.m_errorfolder = g_prefs.m_ImportConfigurations[0].sErrorFolder;
		g_prefs.m_donefolder = g_prefs.m_ImportConfigurations[0].sDoneFolder;
		g_prefs.m_logfolder = g_prefs.m_ImportConfigurations[0].sLogFolder;

		g_prefs.m_senderrormail = g_prefs.m_ImportConfigurations[0].bSendErrorEmail;
		g_prefs.m_emailreceivers = g_prefs.m_ImportConfigurations[0].sEmailReceivers;

		g_prefs.m_inputfolder2 = _T("");

		g_prefs.m_copyfolder = g_prefs.m_ImportConfigurations[0].sCopyFolder;

		if (g_prefs.m_ImportConfigurations.GetCount() > 1) {
			g_prefs.m_inputfolder2 = g_prefs.m_ImportConfigurations[1].sInputFolder;
			g_prefs.m_errorfolder2 = g_prefs.m_ImportConfigurations[1].sErrorFolder;
			g_prefs.m_donefolder2 = g_prefs.m_ImportConfigurations[1].sDoneFolder;
			g_prefs.m_logfolder2 = g_prefs.m_ImportConfigurations[1].sLogFolder;
			g_prefs.m_copyfolder2 = g_prefs.m_ImportConfigurations[1].sCopyFolder;

			g_prefs.m_senderrormail2 = g_prefs.m_ImportConfigurations[1].bSendErrorEmail;
			g_prefs.m_emailreceivers2 = g_prefs.m_ImportConfigurations[1].sEmailReceivers;
